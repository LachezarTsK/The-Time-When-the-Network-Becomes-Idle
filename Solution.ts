...
function findMinTimeWhenTheNetworkBecomesIdle(undirectedGraph: number[][], patience: number[]): number {
    const queue = new Queue<number>();
    const visited: boolean[] = new Array(util.numberOfServers).fill(false);
...
function createUndirectedGraph(edges: number[][]): number[][] {
    const undirectedGraph: number[][] = Array.from(new Array(util.numberOfServers), () => new Array());
...
function calculateTimeForServerToBecomeIdle(distanceFromMasterServer: number, patience: number): number {
